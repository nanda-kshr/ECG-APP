...
try {
    ...
    if ($hasDutyRoster) {
        // Note: some schemas may not have is_active column; join only on doctor_id and today's date
        $sql = "SELECT 
                    ...
                    dr.shift
                FROM users u
                LEFT JOIN duty_roster dr ON dr.doctor_id = u.id AND dr.duty_date = CURDATE()
                WHERE $whereClause
                ORDER BY u.name ASC
